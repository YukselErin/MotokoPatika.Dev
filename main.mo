...
actor {
    ...
    public func listAllPokemons() : async () {
        List.iterate<Pokemon>(pokedex, func n {Debug.print(n.Name)})
    };

    public func listOwnedPokemons() : async () {
        var index = 1;
        Buffer.iterate<Pokemon>(ownedPokemon, func n {Debug.print(Nat.toText(index) # ". " #n.Name); index :=index+1;})
    };
    ...
    public func visitPokeCenter() : async Text{
        if(isInFight){
            ...
            return("All your pokemone are healed!\n");
        };
    };
